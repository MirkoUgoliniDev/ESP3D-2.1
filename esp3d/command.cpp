...
bool COMMAND::execute_command (int cmd, String cmd_params, tpipe output, level_authenticate_type auth_level, ESPResponseStream  *espresponse)
{
  ...
  String IP;
    ...
    case 400: {
        ...
        if (cmd_params == "printer" || cmd_params == "") {
            ...
            //Sonoff Type
			ESPCOM::print (F ("{\"F\":\"network\",\"P\":\""), output, espresponse);
            ESPCOM::print ( (const char *) CONFIG::intTostr (ESP_SONOFF_TYPE), output, espresponse);
            ESPCOM::print (F("\",\"T\":\"B\",\"V\":\""), output, espresponse);
			...
			ESPCOM::print (F("\",\"H\":\"SonoffType\",\"O\":[{\"None\":\"0\"},{\"Legacy\":\""), output, espresponse);      
            ESPCOM::print ( (const char *) CONFIG::intTostr (ESP_SONOFF_LEGACY), output, espresponse);
			
            ESPCOM::print (F("\"},{\"Tasmota\":\""), output, espresponse);
            ESPCOM::print ( (const char *) CONFIG::intTostr (ESP_SONOFF_TASMOTA), output, espresponse);  
			ESPCOM::print (F("\"}]}"), output, espresponse);	
			ESPCOM::println (F (","), output, espresponse);
			...
			//ESP_SONOFF_IP1 
            ESPCOM::print (F ("{\"F\":\"network\",\"P\":\""), output, espresponse);
            ESPCOM::print ( (const char *) CONFIG::intTostr (ESP_SONOFF_IP1 ), output, espresponse);
            ESPCOM::print (F ("\",\"T\":\"A\",\"V\":\""), output, espresponse);
            if (!CONFIG::read_buffer (ESP_SONOFF_IP1 , (byte *) ipbuf, IP_LENGTH) ) {
                ESPCOM::print ("???", output, espresponse);
            } else {
                ESPCOM::print (IPAddress (ipbuf).toString().c_str(), output, espresponse);
            }
            ESPCOM::print (F ("\",\"H\":\"Sonoff 1\"}"), output, espresponse);
            ESPCOM::println (F (","), output, espresponse);
			...
			//ESP_SONOFF_IP2 
            ESPCOM::print (F ("{\"F\":\"network\",\"P\":\""), output, espresponse);
            ESPCOM::print ( (const char *) CONFIG::intTostr (ESP_SONOFF_IP2 ), output, espresponse);
            ESPCOM::print (F ("\",\"T\":\"A\",\"V\":\""), output, espresponse);
            if (!CONFIG::read_buffer (ESP_SONOFF_IP2 , (byte *) ipbuf, IP_LENGTH) ) {
                ESPCOM::print ("???", output, espresponse);
            } else {
                ESPCOM::print (IPAddress (ipbuf).toString().c_str(), output, espresponse);
            }
            ESPCOM::print (F ("\",\"H\":\"Sonoff 2\"}"), output, espresponse);
            //SPCOM::println (F (","), output, espresponse);
       ...
       Serial.println("Response:");
        ...
        if (response) {
            ...
            if (styp == "A") {
				...
				int len = CONFIG::split_ip (sval.c_str(), ipbuf);
    ...
    case 402: {
        Serial2.println("IP?");
        ESPCOM::println (String(getCAM_IP()).c_str(), output, espresponse);
    }
    break;
	...
	case 403: {	
         ...
         IPAddress ip (ip_buf[0], ip_buf[1], ip_buf[2], ip_buf[3]);	         
	     ESPCOM::println (ip.toString().c_str(), output, espresponse);	     		 
    }
    break;
    ...
    case 466: {   
        ...
        IP = get_param (cmd_params, "P=", false);
        ...
        if (sonoffservice.SetStatus(IP, parameter)){
            ESPCOM::println (OK_CMD_MSG, output, espresponse);
        } else {
            ESPCOM::println (ERROR_CMD_MSG, output, espresponse);
            response = false;
    ...
    //[ESP467]P=<1> 
    case 467: { 
        IP = get_param (cmd_params, "P=", true);    
        ESPCOM::println (String(sonoffservice.GetStatus(IP)).c_str(), output, espresponse);       
    }
    break;
...
#ifdef SONOFF_FEATURE
     //Set/Get Sonoff Settings
     //[ESP609]type=<NONE/LEGACY/TASMOTA>
   //Get will give type
   case 609:  
  #ifdef AUTHENTICATION_FEATURE
        if (auth_type == LEVEL_GUEST) {
  ...
  parameter = get_param (cmd_params, "", false);
        ...
        if (parameter.length() == 0) {
            uint8_t Ntype =  0;
            if (!CONFIG::read_byte (ESP_SONOFF_TYPE, &Ntype ) ) {
                Ntype =0;
            }
            char sbuf[MAX_DATA_LENGTH + 1];
            static String tmp;          
            tmp = (Ntype == ESP_SONOFF_LEGACY)?"LEGACY":(Ntype == ESP_SONOFF_TASMOTA)?"TASMOTA":"NONE";           
            ...
            if (parameter.length() > 0) {
                ...
                if (parameter == "NONE") {
                    Ntype = 0;
                } else if (parameter == "LEGACY") {
                    Ntype = ESP_SONOFF_LEGACY;
                } else if (parameter == "TASMOTA") {
                    Ntype = ESP_SONOFF_TASMOTA;
                } else {
                    ESPCOM::println (INCORRECT_CMD_MSG, output, espresponse);
                    return false;
                }
                if (!CONFIG::write_byte (ESP_SONOFF_TYPE, Ntype) ) {
                    ESPCOM::println (ERROR_CMD_MSG, output, espresponse);
                    return false;
                } else {
                    response = true;
